...
func TestRealisticChineseMixedCorpusAcrossRounds(t *testing.T) {
	...
	seeds := []seed{
		...
		{query: "embedded appliance", receipt: "Caelis Host 直接导入 Memory appliance Go package，不下载或管理 sidecar。"},
		...
		{query: "公开 api sdk internal", receipt: "Agent Host 只导入公开 appliance、api 与 sdk package，不能访问 Memory internal 包。"},
		{query: "Windows named pipe", receipt: "未来独立发行时，Windows 本地传输可采用 owner-restricted named pipe。"},
		{query: "Darwin Linux", receipt: "未来独立发行时，Darwin 和 Linux 可使用 owner-only Unix Domain Socket。"},
		{query: "external review", receipt: "正式 GA 之前必须经过 external review 暂停点。"},
		{query: "memoryctl operator", receipt: "memoryctl 属于暂缓的独立发行框架，不是 Caelis 内嵌记忆入口。"},
		{query: "Plugin post-GA", receipt: "Plugin 是延期的生态集成面，复用稳定 Memory API 契约。"},
		{query: "Generator callback", receipt: "raw-memory-016", canonical: "下游通过 Generator callback 注入既有模型能力。"},
		{query: "claim apply fail", receipt: "raw-memory-017", canonical: "下游 Worker callback 只使用 claim apply fail 三类操作。"},
		...
		{query: "provider configuration", receipt: "raw-memory-021", canonical: "provider configuration 完全属于下游，不进入 Memory package。"},
