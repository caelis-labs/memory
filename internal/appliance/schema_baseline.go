...
var baselineSchema = []string{
	...
	`INSERT INTO metadata(key, value) VALUES ('schema_baseline', '` + schemaBaselineID + `')`,
	...
	`CREATE TABLE capabilities (
		...
		label_set_digest TEXT NOT NULL DEFAULT '` + emptyLabelSetDigest + `'
	) STRICT`,
	...
	`CREATE INDEX receipts_space_label_sequence
	 ON receipts(space_id, label_set_digest, commit_sequence DESC)`,
	...
	`CREATE INDEX steward_jobs_space_label_state
	 ON steward_jobs(space_id, label_set_digest, state)`,
	`CREATE INDEX semantic_records_space_status
	 ON semantic_records(space_id, status, updated_at)`,
	`CREATE INDEX semantic_records_space_label_status
	 ON semantic_records(space_id, label_set_digest, status, updated_at)`,
	`CREATE INDEX semantic_evidence_receipt
	 ON semantic_evidence(receipt_id, record_id, revision)`,
	`CREATE INDEX lexicon_terms_space_status
	 ON lexicon_terms(space_id, status, score DESC, term)`,
	`CREATE INDEX lexicon_evidence_receipt
	 ON lexicon_term_evidence(receipt_id, space_id, term)`,
}

func initializeSchema(ctx context.Context, db *sql.DB, now time.Time) error {
